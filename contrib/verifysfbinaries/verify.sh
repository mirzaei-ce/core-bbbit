...
###   This script attempts to download the signature file SHA256SUMS.asc from bbbit.org
...
WORKINGDIR="/tmp/bbbit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://bbbit.org/bin/"
VERSIONPREFIX="bbbit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bbbit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Bbbit Core binary release signing key installed?"
   fi
